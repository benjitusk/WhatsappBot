...
class Bot {
    constructor(timeout = undefined) {
        this.FAKE_DATABASE = process.env.PRODUCTION
            ? '/home/dinnerBot/whatsapp.json'
            : '../dinnerSite/whatsapp.json';
        ...
        if (timeout)
            setTimeout(() => {
                console.log(
                    `Restarting process becuase we exceeded the maximum lifetime of ${
                        this.maximumLifetime / 600
                    } minutes`
                );
                process.exit(0);
...
"What's for dinner?" has been asked ${
            database.dinner.count
        } times, the most recent of which being ${prettyMilliseconds(
            Date.now() - database.dinner.lastMention,
...
14 people lost their AirPods  so far.`;
    ...
    newTask(action, user, chat, dueDate) {
        ...
        console.log(
            `Adding the following task to the database: ${JSON.stringify(
                task,
                null,
                2
            )}`
            ...
            if (task.dueDate <= now) {
                ...
                switch (task.action) {
                    ...
                    case 'unmuteChat':
                        ...
                        break;
                    case 'unblockUser':
                        this.database.blacklist.splice(
                            this.database.blacklist.indexOf(task.user),
                            1
                        );
                }
                this.database.futureTasks.splice(
                    this.database.futureTasks.indexOf(task),
                    1
                );
                this.writeChangesToFile();
            }
...
class Poll_Manager {
    ...
    publish(chat, type, topic, allowMeh = false, test = false, publish = true) {
        ...
        else if (publish.toLowerCase() === 'false') publish = false;
        let date = new Date();
        ...
        let title = `${type} poll:`;
        let footer =
            'Once you cast your vote, subsequent votes by you will be ignored.';
        let pollID = `${topic.replaceAll(' ', '_')}_${date.getMonth() + 1}/${
            date.getDate() + 1
        }`;
        let buttonArray = [
            { id: `${pollID}:good`, body: '👍' },
        ...
        if (!allowMeh || allowMeh.toLowerCase() === 'false')
        ...
        this.con.query(
            `INSERT INTO poll_master (poll_id, poll_display_name) VALUES ("${pollID}", "${topic}");`
        );
        ...
        this.con.query(
            `CREATE TABLE IF NOT EXISTS poll_${pollID} (id int NOT NULL AUTO_INCREMENT, voter_id text NOT NULL, voter_display_name text NOT NULL, vote text NOT NULL, PRIMARY KEY (id)) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;`
        );
    }
    ...
    handleResult(result) {
        ...
        if (poll.voterIDs.includes(result.sender))
            return result.contactChat.sendMessage(
                'You have already voted on this poll. Subsequent votes will be discarded.'
            );
        ...
        this.con.query(
            `INSERT INTO poll_${pollID} SET voter_id = "${result.sender}", voter_display_name = "${result.displayName}", vote = "${selectedButton}";`
        );
        // update the poll_master table with the new vote
        this.con.query(
            `UPDATE poll_master SET ${selectedButton} = ${selectedButton} + 1 WHERE poll_id = "${pollID}";`
        );
    }
}
