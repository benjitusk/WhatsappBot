...
export class Users {
    ...
    createUserWithChatIfNotExists(userID: string, chatID: string): void {
        if (!this.data[userID] || !this.data[userID][chatID])
            this.data[userID] = { [chatID]: {} };
        this.set(this.data);
    }
    ...
    userHasActiveVoteKick(userID: string, chatID: string): boolean {
        ...
        if (this.data[userID][chatID].voteKick)
            if (this.data[userID][chatID].voteKick!.voteExpires > Date.now())
                ...
                this.deleteVoteKickByID(
                    this.data[userID][chatID].voteKick!.voteKickID
                );
        return false;
    }
    ...
    voteKickVote(
        ...
        if (voteKick.votes.includes(votingUserID)) {
            console.log(
                `User ${votingUserID} already voted for vote kick ${voteKickID}`
            );
            return;
        }
        console.log(
            `User ${votingUserID} is voting for vote kick in chat ${voteKickID}`
        );
        this.data[voteKick.userID][voteKick.chatID].voteKick?.votes.push(
            votingUserID
        );
        this.set(this.data);
    ...
    async applyVoteKick(
        ...
        for (const userID in this.data)
            for (const chatID in this.data[userID])
                if (
                    this.data[userID][chatID].voteKick?.voteKickID == voteKickID
                ) {
                    ...
                    if (voteKick.votes.length >= Users.VOTEKICKCOUNT) {
                        ...
                        const chatContact = (await client.getChatById(
                            chatID
                        )) as GroupChat;
                        this.banUserFromChat(
                            userContact,
                            chatContact,
                            60 * 60 * 1000,
                            'VoteKick'
    ...
    banUserFromChat(
        ...
        if (
            !this.userIsBannedFromChat(user.id._serialized, chat.id._serialized)
        )
        ...
        for (const userID in this.data)
            for (const chatID in this.data[userID])
                if (this.data[userID][chatID].ban)
                    bans.push(this.data[userID][chatID].ban!);
        return bans;
    }
    ...
    unsetBanByID(banID: string): void {
        for (const userID in this.data)
            for (const chatID in this.data[userID])
                if (this.data[userID][chatID].ban?.banID == banID)
                    delete this.data[userID][chatID].ban;
...
export class MishnaYomi {
    ...
    async getNextMishnaAt(requestedMishna: {
        ...
        while (typeof mishnaData === 'string') {
            ...
            if (retryCount > 5) {
                console.error(
                    `[Mishna Yomi] Couldn't get mishnaYomi: ${mishnaData}`
                );
                return null;
            }
            ...
            mishnaData = await this.getSpecificMishna(
                this.getDataForNextMishna()
            );
        }
