...
module.exports = {
    ...
    async execute(message: Message, client: CustomClient): Promise<void> {
                ...
                if (chat.isGroup) {
                    ...
                    console.log(
                        `${name} was removed for ${
                            filter.reason
                        }, and will be unbanned in ${
                            filter.timeout / (60 * 60)
                        } ${filter.timeout === 1 ? 'hour' : 'hours'}.`
                    );

                    Users.shared.banUserFromChat(
                        contact,
                        chat,
                        filter.timeout * 1000,
                        filter.reason
        ...
        if (message.body.startsWith(prefix)) {
            ...
            let args = message.body.trim().split(/\s+/);
        ...
        client.autoResponses.forEach(async (autoResponse: AutoResponse) => {
            if (await autoResponse.executeCondition(message)) {
                if (autoResponse.enabled) {
                    ...
                    console.log(
                        `[AutoResponse] ${autoResponse.name} was executed.`
                    );
                } else {
                    console.log(
                        `[AutoResponse] ${autoResponse.name} is disabled.`
                    );
                }
            }
        });
